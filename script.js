...
const logo = document.querySelector(".foot-logo");
...
function updateFooter() {
    ...
    target = (window.innerHeight - rect.top) /
             (window.innerHeight + rect.height);

    target = Math.max(0, Math.min(target, 1));

    // Cubic easing
    target = 1 - Math.pow(1 - target, 3);

    // Smooth interpolation
    current += (target - current) * 0.12;

    const rotate = 88 - current * 88;
    const translateY = 180 - current * 180;
    const scaleY = 0.8 + current * 0.14;
    const scaleX = 0.92 + current * 0.08;
...
const texasTime = document.getElementById("texastime");
...
function updateTexasTime() {
    ...
    texasTime.textContent = `${time} CST`;
}
